...
import newbook.dao.UserLibraryDao;
import newbook.model.Newbook;
import newbook.model.NewbookListView;


public class GetUserNewbookListService {
	
	// MessageListView 를 생성해서 결과로 반환
	
	private GetUserNewbookListService() {}
	
	private static GetUserNewbookListService service = new GetUserNewbookListService();
	
	public static GetUserNewbookListService getInstance() {
		return service;
	}
	...
	public NewbookListView getNewbookListView(int pageNumber, String userid) {
		...
		try {
			...
			UserLibraryDao dao = UserLibraryDao.getInstance();
			
			// 전체 게시물의 개수
			int newbookTotalCount = dao.selectCount(conn, userid);
			...
			if(newbookTotalCount > 0) {
				...
				newbookList = dao.selectList(conn, firstRow, endRow, userid);
